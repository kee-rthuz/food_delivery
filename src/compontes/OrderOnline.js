...
const DineIn = () => {
  ...
  const handleImageClick = (item) => {
    navigate('/menu', { state: { restaurantId: item.id, restaurantName: item.name } });
  };
          ...
          {filteredMenuItems.map((item) => (
            ...
            >
              <img 
                src={item.image} 
                alt={item.name} 
                className="w-full sm:w-24 md:w-32 h-40 sm:h-24 md:h-32 object-cover flex-shrink-0 cursor-pointer transition-transform hover:scale-105 rounded-md"
